...
async def join_channel(event):
	try :
		await abbas(JoinChannelRequest('@RrrruI5'))
	except BaseException:
		pass		
@abbas.on(events.NewMessage)
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@Php_Py_1'))
	except BaseException :
		pass
@abbas.on(events.NewMessage)
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@YYY4YI'))
	except BaseException :
		pass
...
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@zwzwwzz'))
	except BaseException :
		pass
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات حالياً 🤍') != -1:
                ...
                break
            msg_text = msgs.message  # الكود تمت كتابتهُ من قبل سورس منذر ممنوع الخمط
    ...
    while True:
        try:
           ...
           if sender.id == onerabbas_id:
               ...
               #
               channel_entity = await abbas.get_entity(pot)
#               await abbas.send_message(pot, '**جاري بدأ عملية التجميع بواسطة منذر**')
               await abbas.send_message(pot, '/start')
               await asyncio.sleep(2)
               msg0 = await abbas.get_messages(pot, limit=1)
               await msg0[0].click(2)
...
async def Hussein(event):
    ...
    if sender.id == onerabbas_id:
	    abbas = event.client
	    joinu = await abbas(JoinChannelRequest('YYY4YI'))
	    await event.delete()
...
async def Hussein(event):
    ...
    if sender.id == onerabbas_id:
	    abbas = event.client
	    joinu = await abbas(JoinChannelRequest('RrrruI5'))
	    await event.delete()
...
async def stop_aljoker(event):
    ...
    if sender.id == onerabbas_id:
	    global yaAli
	    yaAli = False
	    await event.reply("**᯽︙ تم ايقاف النشر التلقائي بنجاح ✓** ")
